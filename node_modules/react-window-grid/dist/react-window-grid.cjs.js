...
function useMetadataFixed(metadata, clientSize, fillerType, minVisibleScrollViewWidth) {
  ...
  var mass = React.useMemo(function () {
    ...
    if (mass >= 0) {
      metadata = {
        meta: Object.keys(metadata.meta).reduce(function (accum, key) {
          ...
          var _value = tslib.__assign(tslib.__assign({}, value), {
            // @ts-ignore
          ...
          return tslib.__assign(tslib.__assign({}, accum), (_a = {}, _a[key] = _value, _a));
        }, {}),
        // @ts-ignore
        pre: {
          count: 0,
        ...
        // @ts-ignore
        mid: tslib.__assign({}, metadata.total),
        // @ts-ignore
        post: {
          count: 0,
          size: 0,
      ...
      if (mass > 0) {
        if (fillerType === true || fillerType === 'append') {
          ...
          metadata = tslib.__assign(tslib.__assign({}, metadata), {
            // contentSize,
            meta: tslib.__assign(tslib.__assign({}, metadata.meta), (_a = {}, _a[metadata.total.count] = {
              size: mass,
            ...
            total: tslib.__assign(tslib.__assign({}, metadata.total), {
              count: metadata.total.count + 1,
              size: contentSize
            }),
            mid: tslib.__assign(tslib.__assign({}, metadata.total), {
              // @ts-ignore
              count: metadata.total.count + 1,
          ...
          metadata = tslib.__assign(tslib.__assign({}, metadata), {
            meta: tslib.__assign(tslib.__assign({}, metadata.meta), (_b = {}, _b[lastKey] = tslib.__assign(tslib.__assign({}, metadata.meta[lastKey]), {
...
function useRangeHelper(_a) {
  var fixedTopCount = _a.fixedTopCount,
      ...
      getSize = _a.getSize;
  var helper2 = React.useMemo(function () {
    // console.log({
    //   fixedTopCount,
    //   fixedBottomCount,
    //   fixedLeftCount,
...
function useHelpers(props) {
  ...
  var getRange = React.useMemo(function () {
...
function useCachedItem(props) {
  ...
  var cached__tmp = React.useRef({});
  var cached = React.useMemo(function () {
  ...
  var getItemContent = React.useMemo(function () {
  ...
  var getCachedStyle = React.useMemo(function () {
    return function (rowIndex, colIndex, rowTypes, colTypes) {
      ...
      if (!cached.current[key]) {
        ...
        var className = tslib.__spreadArrays([classNames.CELL], rowTypes.map(function (e) {
...
function useClassNames(classNames) {
  var hash = React.useMemo(function () {
  ...
  return React.useMemo(function () {
    return Object.keys(classNames || {}).reduce(function (accum, key) {
      ...
      return tslib.__assign(tslib.__assign({}, accum), (_a = {}, _a[key] = classNames[key], _a));
    }, tslib.__assign({}, CLASSNAMES)); // return JSON.stringify(classNames);
  }, [hash]);
}
...
function getComputedStyle(target, attributes) {
  ...
  return attributes.reduce(function (accum, attribute) {
    ...
    return tslib.__assign(tslib.__assign({}, accum), (_a = {}, _a[attribute] = pixelToNumber(computed[attribute]), _a));
  }, {
...
function useTarget(target) {
  ...
  return React.useMemo(function () {
    ...
    dummy.current.className = typeof target === 'string' ? emotion.css({
      position: 'absolute',
      top: -9999
    }) + ' ' + target : emotion.css(tslib.__assign({
      position: 'absolute',
      top: -9999
    }, target));
...
function useContainerStyle(objectStyles) {
  var objectStylesHash = React.useMemo(function () {
    return JSON.stringify(objectStyles);
  }, [objectStyles]);
  var className = React.useMemo(function () {
    return objectStyles && typeof objectStyles === 'object' ? emotion.css(objectStyles) : '';
  }, [objectStylesHash]);
...
function useContainerInfo(_a) {
  ...
  var className = React.useMemo(function () {
    return [classNames.CONTAINER, theme, containerClassName, emotion.css({
      height: offsetHeight
    })].filter(function (e) {
      return e;
    }).join(' ');
...
function useScrollHelper(rowCount, columnCount, fixedBottomCount, fixedRightCount, getItemMetadata, misc) {
  ...
  var _a = React.useMemo(function () {
    ...
    var update = function (anim) {
      ...
      anim.animations.reduce(function (accum, _a) {
        ...
        return tslib.__assign(tslib.__assign({}, accum), (_b = {}, _b[property] = currentValue, _b));
      }, {});
    };
  ...
  var scrollToIndexFn = React.useRef();
  scrollToIndexFn.current = React.useMemo(function () {
    return function (_a) {
...
function useRange(rowMetadata, overscanRowStartIndex, overscanRowStopIndex, columnMetadata, overscanColumnStartIndex, overscanColumnStopIndex) {
  return React.useMemo(function () {
    return {
      top: tslib.__spreadArrays(rowMetadata.pre.range, rowMetadata.pre.range),
      bottom: tslib.__spreadArrays(rowMetadata.post.range, rowMetadata.post.range),
      left: tslib.__spreadArrays(columnMetadata.pre.range, columnMetadata.pre.range),
      right: tslib.__spreadArrays(columnMetadata.post.range, columnMetadata.post.range),
...
function useTheme(theme, classNames) {
  var dotClassNames = React.useMemo(function () {
    return Object.keys(classNames).reduce(function (accum, key) {
      var _a;
  ...
  return React.useMemo(function () {
    return [defaultTheme(dotClassNames), theme && emotion.css(theme(dotClassNames))].filter(function (e) {
      return e;
    }).join(' ');
  }, [theme, dotClassNames]);
}
...
var WindowGrid = function (props, ref) {
  ...
  var containerInfo = useContainerInfo(tslib.__assign(tslib.__assign({}, props), {
    theme: theme,
  ...
  var helpers = useHelpers(tslib.__assign(tslib.__assign({}, props), {
  ...
  var handleScroll = function (event) {
    ...
    timeoutID.current = setTimeout(function () {
      var _scroll = tslib.__assign(tslib.__assign({}, scroll), {
        isScrolling: false
      });
      ...
      if (misc.current && props.scrollSnap && !scrollHelper.isScrolling() && scrollHelper.scrollTo({
        ...
        _scroll = tslib.__assign(tslib.__assign({}, scroll), {
          isScrolling: true
        });
      }
  ...
  var offsetHeight = React.useMemo(function () {
  ...
  }, center.items))), props.guideline && React.createElement("div", {
...
var WindowGrid$1 = React.forwardRef(WindowGrid);

exports.WindowGrid = WindowGrid$1;
//# sourceMappingURL=react-window-grid.cjs.js.map
